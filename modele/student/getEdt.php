...
try {
    ...
    if (empty($results)) echo "Rien pour aujourd'hui.";
        ...
        // réorganisation des cours par jour :
    ...
    function setDate(bool $today = false): array
    {
        ...
        // cette semaine
            ...
            if ($dayNumber < 6) {
                ...
                $currentDate->modify("-{$daysToSubtract} day"); // devient lundi de cette semaine
            }

            // le weekend
            else {
                $daysToAdd = 8 - $dayNumber;
                $currentDate->modify("+{$daysToAdd} day"); // devient lundi de la semaine prochaine
            }

            // compteur de 5 jours à partir de lundi
            for ($i = 0; $i < 5; $i++) {
                $jour = witchDay($currentDate->format('N'));
                $date = $currentDate->format('Y-m-d');
                $weekdays[$jour] = $date;
